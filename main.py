...
-> """)
    ...
    is_sorted = True if input("Отсортировать операции по дате? (Да, если отсортировать/Любой другой ответ, если нет) ->").lower() == "да" else False
    if is_sorted:
        is_desc_sorted = True if input("Отсортировать по возрастанию или по убыванию?").lower() == 'по убыванию' else False
    is_only_rub_transactions = True if input("Выводить только рублевые транзакции?(Да, если отсортировать/Любой другой ответ, если нет) ->").lower() == "да" else False
    is_sorted_by_word = True if input("Отсортировать список транзакций по определенному слову в описании? (Да, если отсортировать/Любой другой ответ, если нет) ->").lower() == "да" else False
...
{masked_to_requisites if not operation.get("from") else a}
""")
